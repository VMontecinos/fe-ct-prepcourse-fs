...
function arrayContiene(array, elemento) {
   ...
   // Tu código:
   let poseeElemento = array.includes(elemento);
   return poseeElemento;
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   // Tu código:
   const max = arrayOfNums.reduce((a, b) => { return Math.max(a, b)});
   return max;
}
...
function tablaDelSeis() {
   ...
   // Tu código:
   const tablaCreciente = [];

   for (let i = 0; i <= 10; i++) {
      const resultado = 6 * i;
      tablaCreciente.push(resultado);
   }
...
function mayorACien(array) {
   ...
   for (let i = 0; i < array.length; i++) {
      if (array[i] > 100) {
         const nuevoNum = array[i];
         cienOMas.push(nuevoNum);
      }
   }
   return cienOMas;
}
